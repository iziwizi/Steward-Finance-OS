...
import { RefreshCw, Plus, Trash2, Edit3, Loader2, X, Check, Calendar } from "lucide-react";
import { formatNaira } from "@/lib/finance/allocation-engine";
import { createSubscription, updateSubscription, deleteSubscription } from "@/lib/actions/misc";
  ...
  return (
    <div className="space-y-6 pb-12 w-full max-w-full overflow-hidden">
      ...
      <div className="hidden md:flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 md:text-2xl">Subscriptions</h1>
          <p className="text-xs text-zinc-500">
            Track recurring software, streaming services, and monthly digital commitments.
          </p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          <span>{isAdding ? "Cancel" : "Add Subscription"}</span>
        </button>
      </div>
      ...
      <div className="rounded-2xl border border-brand-200/80 bg-brand-50/60 p-5 md:p-6 shadow-xs space-y-2">
        ...
        <p className="text-xs text-zinc-500">
          Across {subs.length} active digital subscription{subs.length === 1 ? "" : "s"}
        </p>
      </div>
      ...
      {isAdding && (
        <div className="rounded-2xl border border-zinc-200 bg-white p-5 md:p-6 shadow-sm animate-in fade-in duration-fast">
          <div className="flex items-center justify-between border-b border-zinc-100 pb-3 mb-4">
            <h2 className="text-sm font-bold text-zinc-900">Add New Subscription</h2>
            <button
              type="button"
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              ...
              <input
                name="service_name"
                placeholder="e.g. Netflix, Spotify, AWS, iCloud"
                required
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  name="category"
                  placeholder="e.g. Entertainment, SaaS, Storage"
                  className="w-full rounded-xl border border-zinc-200 bg-white px-3.5 py-2.5 text-xs text-zinc-900 focus:border-brand-500 focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-zinc-700 mb-1">Plan / Tier</label>
                <input
                  name="plan"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-zinc-700 mb-1">Cost (₦)</label>
                <input
                  name="cost"
                  type="number"
                  step="0.01"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-fast">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl border border-zinc-200 space-y-4 animate-in zoom-in-95 duration-fast">
            <div className="flex items-center justify-between border-b border-zinc-100 pb-3">
              <h2 className="text-sm font-bold text-zinc-900">Edit Subscription</h2>
              <button
                type="button"
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              ...
              <div>
                ...
                <input
                  name="service_name"
                  defaultValue={editingSub.service_name}
                  required
                  className="w-full rounded-xl border border-zinc-200 bg-white px-3.5 py-2.5 text-xs text-zinc-900 focus:border-brand-500 focus:outline-none"
                />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-zinc-700 mb-1">Plan</label>
                  <input
                    name="plan"
                    defaultValue={editingSub.plan || ""}
                    className="w-full rounded-xl border border-zinc-200 bg-white px-3.5 py-2.5 text-xs text-zinc-900 focus:border-brand-500 focus:outline-none"
                  />
                </div>
              </div>
      ...
      <div className="space-y-3">
          ...
          >
            <div className="flex items-center gap-3.5 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-zinc-900 truncate">{s.service_name}</p>
                <p className="text-[11px] text-zinc-400 flex items-center gap-1.5">
                  <span>{s.category || "Subscription"}</span>
                  {s.plan ? (
                    <>
                      <span>•</span>
                      <span>{s.plan}</span>
            ...
            <div className="flex items-center justify-between sm:justify-end gap-4 shrink-0 border-t sm:border-t-0 pt-2 sm:pt-0 border-zinc-50">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (confirm(`Delete "${s.service_name}"?`)) {
                      handleDelete(s.id);
                    }
                  }}
        ...
        {subs.length === 0 && !isAdding && (
          <div className="rounded-2xl border border-dashed border-zinc-200 bg-white p-8 text-center space-y-3">
            ...
            <div>
              <p className="text-xs font-bold text-zinc-800">No subscriptions tracked yet</p>
              <p className="text-[11px] text-zinc-400 mt-0.5 max-w-xs mx-auto">
                Add recurring services like Netflix, Spotify, iCloud, or SaaS tools.
              </p>
            </div>
            <button
              type="button"
